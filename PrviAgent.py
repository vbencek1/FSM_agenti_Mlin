...
class PrviIgrac(Agent):
    ...
    class State3(State):
        async def run(self):
            ...
            if msgD.body.split(";")[0]=="KretanjePloca":
                self.agent.say(f"{msgD.sender} šalje poruku: {msgD.body}")
                self.agent.zabiljeziPozicije(msgD.body)
                pozicije=self.agent.offensiveStrategijaKretanja()
                msg = spade.message.Message(
                        to=str(msgD.sender),
                        body=str(pozicije))
                await self.send(msg)
    ...
    class State4(State):
        async def run(self):
            ...
            if msgD.body.split(";")[0]=="KretanjePloca":
                self.agent.say(f"{msgD.sender} šalje poruku: {msgD.body}")
                self.agent.zabiljeziPozicije(msgD.body)
                pozicije=self.agent.randomStrategijaSkakanja()
                msg = spade.message.Message(
                        to=str(msgD.sender),
                        body=str(pozicije))
                await self.send(msg)
    ...
    async def setup(self):
        ...
        fsm.add_state(name="State3", state=self.State3()) #Kretanje zetonima po loci
    ...
    def makniRandomZeton(self):
        broj=randint(1,24)
        while(True):
            if str(broj) in self.pozicijeDrugog:
                break
            else:
                broj=randint(1,24)
        return broj
    ...
    def randomStrategijaKretanja(self):
        ...
        while(True):
            if str(brojTo) not in self.pozicijePrvog and str(brojTo) not in self.pozicijeDrugog:
                ...
                if str(brojFrom) in self.pozicijePrvog:
                    break
                else:
                    brojTo=randint(1,24)
            else:
    ...
    def offensiveStrategijaKretanja(self):
        slobodnePozicije=[]
        for x in range(1,25):
        ...
        for x in slobodnePozicije:
            ...
            for poz in upit:
                if poz['Y'] in self.pozicijePrvog and poz['Z'] in self.pozicijePrvog:
                    upit2=self.kb.query("pozicija(X,"+str(x)+").")
                    for y in upit2:
                        if str(y['X']) in self.pozicijePrvog and y['X']!=poz['Y'] and y['X']!=poz['Z']:
                            self.say("Aktivirana strategija ofenzive")
                            return str(y['X'])+";"+str(x)
        return self.randomStrategijaKretanja()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Primjer pokretanja: %(prog)s -p /home/vjezbe/software/Flora-2/XSB/bin/xsb -jid vbencek@localhost -pwd 123456")
    parser.add_argument("-p", "--putanja", type=str, help="Putanja do XSB", default="/home/vjezbe/software/Flora-2/XSB/bin/xsb")
    parser.add_argument("-jid", type=str, help="JID Prvog Igraca", default="vbencek@localhost")
    parser.add_argument("-pwd", type=str, help="Lozinka Prvog Igraca", default="123456")
    args = parser.parse_args()
    agentAutomaton = PrviIgrac(args.jid, args.pwd, xsb=args.putanja)
